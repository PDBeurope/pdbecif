...
#        self.foo = None
    ...
    def test_setCategory(self):
        ...
        self.assertEqual(cat_2, self.db.getCategory("_bar"), "Category not set by registration using object")
    ...
    def test_getCategory(self):
        ...
        self.assertIsInstance(self.db.getCategory("_foo"), Category, "Get category did not return Category")
        self.assertEqual(cat, self.db.getCategory("_foo"), "Category name not set correctly")
    ...
    def test_setSaveFrame(self):
        ...
        self.assertIsInstance(save_1, SaveFrame, "setSaveFrame did not return SaveFrame")
        self.assertEqual(save_1.id, "_foo", "SaveFrame name not set correctly")

        save_2 = SaveFrame("_bar", self.db)
        self.assertEqual(save_2, self.db.getSaveFrame("_bar"), "SaveFrame not set by registration using object")

        save_3 = self.db.setSaveFrame(SaveFrame("_bundy", self.db))
        self.assertIsInstance(save_3, SaveFrame, "setSaveFrame did not return SaveFrame")

    def test_getSaveFrame(self):
        save = self.db.setSaveFrame("_foo")
        self.assertIsInstance(self.db.getSaveFrame("_foo"), SaveFrame, "getSaveFrame did not return SaveFrame")
        self.assertEqual(save, self.db.getSaveFrame("_foo"), "SaveFrame name not set correctly")
    ...
    def test_removeChildBadRef(self):
        msg = "DataBlock.removeChild"
        self.db.setCategory("foo")
        self.assertFalse(self.db.removeChild("FAIL"), msg+" did not return expected False" )
