import unittest
from mmcif.mmcif import DataBlock, CifFile
...
class CifFileTestCase(unittest.TestCase):
    ...
    def test_removeChildByString(self):
        ...
        self.assertTrue(
            self.cf.removeChild("TEST"), msg + " did not return expected True"
        )
        self.assertListEqual(
            self.cf.getDataBlocks(), [], msg + " datablocks should be an empty list"
        )
        self.assertIsInstance(
    ...
    def test_removeChildBadRef(self):
        ...
        msg = "CifFile.removeChild"
        self.assertFalse(
            self.cf.removeChild("FAIL"), msg + " did not return expected False"
        )
    ...
    def test_dictionaryImport(self):
        ...
        self.assertIsNotNone(
            cf.getDataBlock("TEST_BLOCK_2"), "CifFile failed to import dictionary"
        )
        self.assertIsInstance(
    ...
    def test_initializeWithDictionary(self):
        ...
        self.assertIsInstance(
            cf, CifFile, "CifFile.__init__ failed to import dictionary"
        )
        self.assertIsNotNone(
            cf.getDataBlock("TEST_BLOCK_2"),
            "CifFile.__init__ failed to import dictionary",
