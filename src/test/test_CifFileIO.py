...
import os
from mmcif.mmcif import CIFWrapper, CifFile
from mmcif.mmcif_io import CifFileReader, CifFileWriter
...
class CifFileIOTestCase(unittest.TestCase):
    ...
    def test_ignoreCategories(self):
        cfr = CifFileReader(input="data", preserve_order=True)
        cif_file = cfr.read(
            self.TEST_CIF_FILE, output="cif_file", ignore=["_test_loop_2", "_valid_CIF"]
        )
        self.assertIsInstance(
            cif_file, CifFile, "Failed to create CifFile using algorithmic parser"
        )
    ...
    def test_write_ciffile_after_ciffile_dictionary_import(self):
        ...
        cfr = CifFileReader(input="data", preserve_order=True)
        test_file = cfr.read(
            os.path.join(self.FILE_ROOT, unit_test_file), output="cif_wrapper"
        )
    ...
    def test_write_ciffile_after_cifwrapper_dictionary_import(self):
        ...
        cif_obj = dict(
            (k, CIFWrapper(v, preserve_token_order=True))
    ...
    def test_write_raw_dictionary_without_block_id(self):
        ...
        data_block_ids.sort()
        self.assertEqual(
            data_block_ids, [unit_test_file], "Datablock(s) were not written correctly"
        )
        self.assertEqual(
            test_file[unit_test_file]._test_category_2.test_value_1,
            ["1", "2", "3", "4"],
            "mmCIF data  was not written correctly",
        )
    ...
    def test_write_mmCIF_dictionary(self):
        ...
        cfw.write(cif_file)
        test_file = cfr.read(
            os.path.join(self.FILE_ROOT, unit_test_file), output="cif_file"
        )
        test_value = (
            test_file.getDataBlock("TEST")
        ...
        self.assertEqual(
            test_value, "no", "mmCIF dictionary file was not written correctly"
        )
