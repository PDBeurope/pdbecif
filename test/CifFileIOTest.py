...
class  CifFileIOTestCase(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        for file in glob(os.path.join(self.path, 'io_testcase*')):
            os.unlink(file)
    ...
    def test_ignoreCategories(self):
        cfr = CifFileReader(input='data')
        cif_file = cfr.read(self.path + "/../../resources/usage-example.cif", output='cif_file', ignore=['_test_loop_2', '_valid_CIF'])
        self.assertIsInstance(cif_file, CifFile, "Failed to create CifFile using algorithmic parser")
        self.assertListEqual(cif_file.getDataBlockIds(), ["TEST_CIF", "BLOCK_2"], "DataBlocks not read correctly")
    ...
    def test_openGzipError(self):
        ...
        self.assertIsNone(cfw._handle, "Failure to open a file should return None")
        
    def test_write_ciffile_after_ciffile_dictionary_import(self):
        # Test write CifFile initialized by dictionary
        unit_test_file = "io_testcase_1.cif"
        cfw = CifFileWriter()
        cif_file = CifFile(os.path.join(self.path, unit_test_file))
        cif_file.import_mmcif_data_map(self.raw_dictionary)
        cfw.write(cif_file)
        cfr = CifFileReader(input='data')
        test_file = cfr.read(os.path.join(self.path, unit_test_file), output='cif_wrapper')
    ...
    def test_write_ciffile_after_cifwrapper_dictionary_import(self):
        ...
        cfw = CifFileWriter(file_path=os.path.join(self.path, unit_test_file))
    ...
    def test_write_mmCIF_dictionary(self):
        ...
        cfr = CifFileReader(input='dictionary')
        cif_file = cfr.read(self.path + "/../../resources/usage-example.dic", output='cif_file')
        cfw = CifFileWriter(file_path=os.path.join(self.path, unit_test_file))
        cfw.write(cif_file)
        test_file = cfr.read(os.path.join(self.path, unit_test_file), output='cif_file')
